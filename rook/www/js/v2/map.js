...
function Map() {
  ...
  chart.draw_data = function(data, g) {
    ...
    var width  = chart.axes.x.canvas_width();
    var canvas_prop = width / height;
    ...
    if (canvas_prop < prop) {
      ...
      var xoffset = 0;
      var yoffset = (chart.axes.y.height() - height) / 2;
      
    } else {
      var scale  = height/(ymax - ymin);
      width  = Math.ceil(height*prop);
      var xoffset = (chart.axes.x.width() - width) / 2;
      var yoffset = 0;
    }

    projection = function(coordinates) {
      return [
          xoffset + Math.round(scale*(coordinates[0]-xmin)), 
          yoffset + Math.round(height-scale*(coordinates[1]-ymin)), 
        ];
    }
