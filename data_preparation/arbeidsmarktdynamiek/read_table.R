...
read_table <- function() {
    data <- read.csv2("labourmarket_mobility.csv")
    ...
    data$personal_characteristics <- factor(data$personal_characteristics, 
        levels=c(
        "Job level: elementary",
        "Job level: lower",
        "Job level: middle",
        ...
        "Education level: unknown",
        "TOTAL"))
    data$position_begin <- factor(data$position_begin, 
        levels=c( "employed", "unemployed", "non labourforce", "TOTAL"))
    data$position_end <- factor(data$position_end, 
        levels=c( "employed", "unemployed", "non labourforce", "TOTAL"))
